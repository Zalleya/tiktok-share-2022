    ...
    try:
        ...
        try:
            if req.json()["status_code"] == 0:
                ...
                TotalSendedShare += 1
                if DebugMode:
                    print(Colorate.Horizontal(Colors.red_to_green, f"Sent Share: {TotalSendedShare} ({impr_id})"))
                else:
                    print(Colorate.Horizontal(Colors.red_to_green, f"Sent Share: {TotalSendedShare} ({impr_id})"))
...
if __name__ == "__main__":
    ...
    NThread = Write.Input("Thread Amount > ", Colors.blue_to_purple, interval=0.0001)
